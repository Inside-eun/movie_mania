  ...
  async crawlKOBIS(): Promise<Movie[]> {
    ...
    try {
      ...
      const scheduleData = await page.evaluate(() => {
        ...
        const items = Array.from(document.querySelectorAll('.schedule-item, tr'));
        console.log('발견된 항목:', items.length);
        return items.map(item => {
          const data = {
            movieName: item.querySelector('.movie-name, td:nth-child(1)')?.textContent?.trim(),
            screenName: item.querySelector('.screen-name, td:nth-child(2)')?.textContent?.trim(),
            showtime: item.querySelector('.showtime, td:nth-child(3)')?.textContent?.trim(),
            theaterName: item.querySelector('.theater-name')?.textContent?.trim() || '마포구 영화관'
          };
        ...
        scheduleData.forEach((item: any) => {
          ...
          movies.push({
            ...
            theater: {
              ...
              location: '서울시 마포구',
              type: item.theaterName.includes('아트') ? 'arthouse' : 'multiplex'
            },
            showtime: new Date(today.getFullYear(), today.getMonth(), today.getDate(), 
