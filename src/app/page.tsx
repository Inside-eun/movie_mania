...
export default function Home() {
  ...
  const handleMovieFilter = (movieTitle: string) => {
    ...
    if (movieTitle === "all") {
      ...
      setFilteredMovies(
        allMovies.filter((movie) => movie.title === movieTitle)
      );
    }
  };
  ...
  return (
    <main className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <p className="text-sm text-gray-500 text-center">
        kobis에서 제공되는 스케쥴을 하나하나 조회하는 방식이라 초기 로딩시간이
        꽤 걸립니다{" "}
      </p>
      <p className="text-sm text-gray-500 mb-8 text-center">
        대략적인 스케쥴의 감을 잡을때만 사용해주세요
      </p>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-2 md:gap-3">
            {filteredMovies.map((movie, index) => (
              <div
                key={index}
                className="p-2 md:p-3 border rounded-lg shadow-sm hover:shadow-md transition-shadow bg-white"
              >
                <div className="flex justify-between items-start mb-1 md:mb-2">
                  <time className="text-xs md:text-sm font-bold text-blue-600 bg-blue-50 px-1 md:px-2 py-1 rounded">
                    {movie.time}
                  </time>
                  {movie.source && (
                    <span className="px-1 md:px-2 py-1 bg-green-100 text-green-800 text-xs rounded">
                      {movie.source}
                    </span>
                  )}
                </div>
