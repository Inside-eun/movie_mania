...
> ⚠️ **주의**: Cron Jobs는 모든 플랜에서 사용 가능하지만, 플랜별 제한이 있습니다:
> - **Hobby 플랜**: 하루 1회만 실행 가능 (위 설정은 배포 실패)
> - **Pro/Enterprise 플랜**: 분 단위 실행 가능 (위 설정 정상 작동)

### 4단계: Hobby 플랜용 Cron 설정 (선택사항)

Hobby 플랜은 하루 1회만 실행 가능하므로 `vercel.json` 수정:
